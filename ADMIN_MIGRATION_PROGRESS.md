...
- Import both JSON files into Postman and select the Stage 3B LOCAL ONLY environment. Keep the supplied baseUrl/local administrator email default (adjust the email privately if needed); enter superAdminPassword only as a private local secret value. All tokens, user/notification IDs, recipientType, runId, runBlocked and helper fields are blank in the template and generated automatically. Never export populated environments or save/share response examples and sensitive console/network captures.
- Start the local backend privately and ensure at least one eligible active customer or technician exists. Run the complete collection in Collection Runner, sequential order, one iteration, beginning at 01. The first request clears stale runtime values, preserves credentials and generates a unique runId. Do not run individual requests or continue after failure. Missing prerequisites stop the run with assertion guidance; fix locally and restart at 01.
- Eleven named steps: health; admin email/password login; current user ID; both active directory lists (automatic pagination, customer preferred with technician fallback); create recipient IN_APP record; create self IN_APP record; current admin inbox; PENDING filter; mark self read; READ filter; final integrity. Inbox steps traverse matching pages to verify ownership and locate the self record. Directory userId maps to recipientUserId, never a profile ID. No typed IDs, fake data or sent-history endpoint.
- New records start PENDING with null scheduledAt/sentAt/readAt. PUT /api/v1/notifications/{id}/read must return the same self record with READ and a valid readAt; later GET comparisons accommodate only sub-microsecond write-to-MySQL precision conversion and then retain the persisted timestamp. All history-free notification integrity checks use actual returned IDs/status/content. Both created records remain for audit; no deletion or cleanup mutation occurs.
- Final recipient assertions refer to the captured create response and absence from the admin inbox. They do not claim verification of the other recipient's login/inbox or later state. No role-negative or future schedule visibility test is claimed. Backend has no admin sent-history, bulk delivery, external email/SMS/push delivery or unread-count support; none is invented.
- Tests assert ApiResponse and positive IDs, expected notification state, selected recipient mapping, exact compose payload fields, page/status filtering, null timestamps, own-inbox behavior, read transition and secret-field exclusion. Required assertion failures set runBlocked and stop. No scripts log credentials or tokens; only login response token fields are allowed and stored privately at runtime.
- Validation: both JSON files parse, all 22 scripts parse, 11 local v1 requests verified, runtime/secret template values blank. All 11 response scripts passed simulated fail-fast checks; a complete simulated run passed including both directories, inbox pagination and timestamp precision. Full npm test: 76 passed, zero failures/skips; npm run build passed. This collection has NOT been executed against live MySQL.
- Browser checks still required: real inbox/filter/paging and empty/timestamp rendering; directory-backed recipient selection and IN_APP compose validation; separation of personal inbox from recipient creation; mark-read refresh and filtered-row behavior; errors without stale/fake data; intended /api/v1 traffic and no red console errors. API collection success alone cannot accept these UI observations.
- Only the two new templates and this document changed. Existing README.md remains preserved/unstaged; existing user-owned Postman environments/secrets were not read or modified. No backend, migration/schema, database, other client or website changes; no push.

# Admin Portal Stage 3B notifications implemented - 11 September 2026

- Existing master; Stage 3A remains accepted. Notification implementation is complete with automated validation; live browser/MySQL verification remains pending. Read the backend notification contract before implementation. No dependencies added.
- My notification inbox: GET /api/v1/notifications with page=0,size=20 and optional PENDING/SENT/FAILED/READ filter. Displays only backend-returned due notifications for the signed-in recipient; title/message, channel, status and nullable scheduled/sent/read timestamps are rendered as escaped text. No unread count or sent-history list is invented.
- Mark read: PUT /api/v1/notifications/{id}/read, exposed only for non-READ rows belonging to the current user. Requires a matching READ response with readAt, then reloads the backend page (including current status filter). No optimistic read-state mutation. Backend remains authoritative for ownership/due-time checks and idempotence. Errors clear visible data and offer refresh.
- Compose: ADMIN/SUPER_ADMIN; reuses existing paginated searchable customer/technician directory picker and GET /api/v1/admin/customers or /admin/technicians with active=true. Recipient userId, not profileId, maps to recipientUserId; there is no manually typed ID input. POST /api/v1/notifications submits recipientUserId,title,message,channel=IN_APP,scheduledAt (nullable). Title/message enforce nonblank and 200/2000-character limits. Optional date/time is explicitly labeled UTC and validated before submission; no implicit local-time conversion. Directory selections reset when switching recipient type.
- Create confirmation reports only the backend record ID/status and distinguishes the recipient's inbox from the logged-in admin inbox. It never appends a notification sent to another user to the admin's inbox or claims provider delivery. Response projection excludes all unrelated and secret/session fields.
- Reuses the centralized API/session/error client, existing shared error presentation and existing pickers/paging without modifying the completed dashboard or workflow modules. 401 refresh/cleanup and separate 403 handling remain centralized. Loading, empty, invalid-input and backend-error states are explicit; stale fetch responses are ignored after filter/navigation changes.
- Intentionally unavailable: sent history, email/SMS/push delivery, providers/workers, bulk send, unread totals, deletion, delivery-state management and recipient selection outside the supported customer/technician directories. Legacy notification modules remain unimported; only the canonical new module is active.
- Validation: full npm test passed with 76 tests, zero failures/skips (64 existing plus 12 notification/API/view cases); npm run build passed. No configured lint/type-check scripts. Tests cover paging/status, empty/nulls, create/user-ID mapping, validation/UTC scheduling, mark-read response, secret exclusion, shared 401 refresh, 403/backend failures, role/read controls, safe text rendering and source scans. An initial source-scan false positive on the local API module import was corrected before the full successful rerun.
- Files: src/api/notifications.js, src/notificationModules.jsx, src/api/runtime.js, src/main.jsx, test/notifications.test.js, test/views.test.js and this record. No changes to accepted workflow/dashboard implementation files or existing Postman files.
- Next: live verification of personal inbox/filter/paging, recipient selection, IN_APP creation/scheduling and own-notification read behavior; automated tests do not establish browser/MySQL acceptance. No live verification performed in this task.
- Safety: README.md remains preserved/unstaged and excluded. Backend was inspected read-only; no backend, migration, database, Android, technician app, website or private environment changes. valor_lift_db untouched. No secrets logged/committed; no branch operation or push.
...
- Confirmed collection weakness: JSON.stringify equality compared complete serialized history objects. Backend event timestamps originate in LocalDateTime.now() while MySQL persists DATETIME(6); write/read precision can differ. The screenshot alone does not identify the exact differing field, so timestamp precision is a source-supported explanation rather than a captured live-response diagnosis.
- Replaced serialized equality in all history comparisons with event-ID/field comparisons. IDs, actors, statuses and notes remain exact; counts and duplicate detection remain enforced. Timestamps allow less than one microsecond difference only when a compared timestamp has more than six fractional digits, supporting storage rounding/truncation and second rollover. Different timestamps already at stored precision remain failures. After a verified GET, snapshots are refreshed to persisted values. No business-state, history-growth or fail-fast checks were removed.
- Validation: 64 portal tests passed (58 existing plus six Postman regression cases); production build passed. Tests cover precision/rollover/property order and rejection of changed notes, actors, statuses, IDs, counts, duplicates and persisted timestamps. All collection scripts parse. No backend/database/UI code changed; README and the environment template remain untouched.
- Re-import the updated collection, keep the existing private local environment selected, and run from 01 for one iteration. Do not resume the blocked request 12 manually. The earlier request remains retained for audit; the new run creates a new uniquely labeled request. Live rerun is pending. If the assertion still fails, provide only sanitized history field differences; do not share authentication responses or headers.
