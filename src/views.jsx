...
import { Activity, AlertTriangle, Building2, CheckCircle2, ClipboardList, HeartPulse, RefreshCw, Server, ShieldCheck, Users, Wrench } from 'lucide-react';
...
function BackendStatus({health, healthLoading, healthError, refresh}) {
  ...
  return <section className={`panel backend-status-card ${healthError ? 'is-error' : health ? 'is-connected' : ''}`}><div className="backend-status-head"><div className="backend-status-icon" aria-hidden="true">{healthError ? <AlertTriangle size={20}/> : healthLoading ? <HeartPulse size={20}/> : <Server size={20}/>}</div><div><h2>Backend API</h2><span className="backend-state">{label}</span></div><button className="secondary-btn" disabled={healthLoading} onClick={refresh}>Check</button></div>
    {healthError ? <div className="backend-status-body" role="alert"><span className="backend-field-label">Connection problem</span><strong>{healthError.message}</strong></div> : health ? <dl className="backend-detail-list"><div><dt>Backend URL</dt><dd>{health.backendUrl}</dd></div><div><dt>Last successful check</dt><dd>{new Date(health.checkedAt).toLocaleString()}</dd></div></dl> : <div className="backend-status-body" role="status"><span className="backend-field-label">Status check</span><strong>Checking backend connectivity...</strong></div>}
  </section>;
}
export function SummaryView({data, loading, error, refresh, health, healthLoading, healthError, refreshHealth}) {
  return <><div className="page-header dashboard-hero"><div><div className="eyebrow">OPERATIONS OVERVIEW</div><h1>Dashboard</h1><p>Current totals from Valor. Counts include retained records and backend-authoritative lifecycle state.</p></div><button className="secondary-btn" disabled={loading || healthLoading} onClick={() => { refresh(); refreshHealth(); }}><RefreshCw size={15}/>Refresh</button></div>
    <div className="dashboard-overview-grid"><BackendStatus health={health} healthLoading={healthLoading} healthError={healthError} refresh={refreshHealth}/></div>
