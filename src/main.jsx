...
import { Home, ClipboardList, AlertTriangle, CalendarDays, Wrench, Users, Building2, Layers3, ShieldCheck, CreditCard, ReceiptText, Package, RefreshCw, Bell, BarChart3, Download, UserRound, Database, Settings, LogOut, Megaphone } from 'lucide-react';
import './styles.css';
...
const navGroups = [
  { id: 'operations', label: 'OPERATIONS', icon: ClipboardList, items: [
  ...
  { id: 'assets', label: 'ASSETS', icon: Building2, items: [
...
const sectionFor = id => navGroups.find(section => section.items.some(item => item.id === id)) || null;
const firstRoute = section => section.items[0]?.id || 'dashboard';
function SectionTabs({section, active, activate}) {
  if (!section) return null;
  return <nav className="section-tabs" aria-label={`${section.label} pages`}>
    {section.items.map(item => <button key={item.id} className={`section-tab ${active === item.id ? 'active' : ''}`} aria-current={active === item.id ? 'page' : undefined} onClick={() => activate(item.id)}>{item.label}</button>)}
  </nav>;
}
  ...
  return <ProtectedContent user={user}><div className="app">
    <aside className={`sidebar ${mobileNav ? 'mobile-open' : ''}`}>
      ...
      <nav className="nav" aria-label="Admin navigation">
        <button title="Dashboard" className={`nav-item nav-dashboard ${active === 'dashboard' ? 'active' : ''}`} onClick={() => activate('dashboard')}><Home size={18}/><span>Dashboard</span></button>
        {navGroups.map(section => {
          const Icon = section.icon;
          return <button key={section.id} title={section.label} className={`nav-item ${activeSection?.id === section.id ? 'active' : ''}`} onClick={() => activateSection(section)}><Icon size={17}/><span>{section.label}</span></button>;
        })}
      </nav>
